...
import {
  loadCryptoUserData as loadUserData,
  saveCryptoBacktests as saveBacktests,
  saveCryptoFolders as saveFolders,
  saveCryptoCurrentBacktest as saveCurrentBacktest,
  saveCryptoTags as saveTags,
  saveCryptoTrackingSessions as saveTrackingSessions,
  saveCryptoTradeFormDefaults as saveTradeFormDefaults,
  listenToCryptoUserData as listenToUserData,
  migrateCryptoFromLocalStorage as migrateFromLocalStorage,
  forceSaveCrypto as forceSave
} from './cryptoFirestoreData.js';
import { testFirestoreConnection } from './testFirestore.js';
import MT5DashboardContent from './components/MT5DashboardContent';
import CryptoLiveTrades from './components/crypto/CryptoLiveTrades';
import { doc, setDoc } from 'firebase/firestore';
import { db } from './firebase';
// Theme Colors - Crypto Theme (Orange/Bitcoin)
const themeColors = {
  // Primary Colors - نارنجی برای کریپتو
  primary: '#ea580c',        // نارنجی اصلی
  primaryLight: '#f97316',   // نارنجی روشن
  primaryDark: '#c2410c',    // نارنجی تیره

  // Success/Profit - نارنجی به جای سبز
  success: '#ea580c',        // نارنجی
  successLight: '#f97316',
  successDark: '#c2410c',
...
export default function CryptoJournalApp({ onBack, liveMode = false, exchangeInfo = null }) {
  // Crypto mode is always false (no MT5 integration for crypto)
  const mt5Mode = false;
  const isSharedView = false;
  const sharedBacktestData = null;
  // Authentication
  ...
  const stats = useMemo(() => {
    ...
    // محاسبه Max Drawdown از بالانس اولیه (نه از Peak)
    let maxDD = 0;
    ...
    ct.slice().reverse().forEach(t => {
      ...
      // اگر زیر بالانس اولیه رفت، drawdown محاسبه کن
      if (currentEquity < initialBalance) {
        const dd = ((initialBalance - currentEquity) / initialBalance) * 100;
        if (dd > maxDD) {
          maxDD = dd;
        }
      }
    ...
    const sortedTrades = [...ct].sort((a, b) => {
      const dateCompare = a.date.localeCompare(b.date);
      if (dateCompare !== 0) return dateCompare;
  ...
  const equityCurve = useMemo(() => {
    ...
    sortedTrades.forEach((t, i) => {
      ...
      // به‌روزرسانی peak (برای نمودار)
      if (eq > peakEquity) {
        peakEquity = eq;
      }

      // محاسبه Max Drawdown از بالانس اولیه (نه از Peak)
      // محاسبه به صورت دلاری و تبدیل به درصد از بالانس اولیه
      if (eq < startBalance) {
  ...
  const selectedMonthlyReport = useMemo(() => {
    ...
    sortedTrades.forEach(t => {
      ...
      // محاسبه Max Drawdown از بالانس اول ماه (نه از Peak)
      // محاسبه به صورت دلاری و بعد تبدیل به درصد از بالانس اولیه
  ...
  const dailyDrawdown = useMemo(() => {
    ...
    filteredTrades.forEach(t => {
      if (!dailyLosses[t.date]) {
        dailyLosses[t.date] = 0;
      }
      if (t.result === 'loss') {
        dailyLosses[t.date]++;
      }
    ...
    // پیدا کردن بیشترین Loss در یک روز
    const maxDailyLosses = Math.max(...Object.values(dailyLosses), 0);

    // محاسبه میانگین ریسک
    const avgRisk = filteredTrades.length > 0
      ? filteredTrades.reduce((sum, t) => sum + t.risk, 0) / filteredTrades.length
      : 1;

    return (maxDailyLosses * avgRisk).toFixed(0);
  }, [getFilteredTrades, selectedMonth]);
    ...
    try {
      ...
      const shareData = {
        ...
        trades: currentBt.trades.map(t => ({
          symbol: `${t.position} @ ${t.rrRatio}RR`,
  ...
  const tabs = currentUser
    ? [
        ...
        ...(mt5Mode ? [{ id: 'mt5Live', label: 'MT5 Live', icon: Activity }] : []),
        ...((typeof liveMode !== 'undefined' && liveMode) ? [{ id: 'liveTrades', label: 'Live Trades', icon: Activity }] : [])
      ]
    : [
        { id: 'dashboard', label: 'Dashboard', icon: BarChart3 },
        { id: 'stopAnalysis', label: 'Stop Analysis', icon: Target },
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold flex items-center gap-2">
                  <span className="bg-gradient-to-r from-orange-400 via-yellow-400 to-green-400 text-transparent bg-clip-text">
                    Top In Trade / Crypto Trading Journal
                  </span>
                </h1>
            ...
            <div className="flex items-center gap-2">
              ...
              {currentUser && !isSharedView && (
                <div className="relative">
                  <button
                    ...
                    className="bg-orange-900 hover:bg-orange-600 p-2 rounded-lg text-sm font-medium transition"
                  >
                    ...
                    <div className="absolute right-full mr-2 top-0 flex items-center gap-2 z-50">
                      ...
                      {!false && (
                        <button
                          ...
                          className="bg-orange-900 hover:bg-orange-600 px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 whitespace-nowrap shadow-xl"
                        >
                      ...
                      {!false && (
                        <label className="bg-orange-600/80 hover:bg-orange-600 px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 cursor-pointer whitespace-nowrap shadow-xl">
              ...
              {!isSharedView && currentUser?.email?.toLowerCase() === 'titteam.1404@gmail.com' && (
                <button
                  ...
                  className="bg-orange-600 hover:bg-orange-700 px-3 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2"
                  title="پنل مدیریت"
                >
                      ...
                      {folder.isExpanded && (
                        <div className="pl-6 pr-2 pb-2 space-y-1">
                          {fbt.length === 0 ? <div className="text-xs text-gray-500 italic py-2 pl-2">Empty folder</div> : fbt.map(bt => {
                            ...
                            return (
                              <div key={bt.id} className={`flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-sm cursor-pointer transition group ${currentBacktest === bti ? 'bg-orange-600/80 text-white' : 'bg-gray-700/50 text-gray-300 hover:bg-gray-600/50'}`}>
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                                  ...
                                  <button onClick={(e) => { e.stopPropagation(); setBacktestToMove(bt); setShowMoveModal(true); }} className="text-green-400 hover:text-green-400 p-1" title="Move backtest"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg></button>
          ...
          <div className="flex-1 overflow-y-auto h-full">
            <div className="p-4 h-full flex flex-col">
              <div className="flex items-center justify-between gap-4 mb-4 overflow-x-auto pb-2">
                <div className="flex gap-2">{tabs.map(tab => { const Icon = tab.icon; return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap ${activeTab === tab.id ? 'bg-orange-600/80 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}><Icon className="w-3 h-3" />{tab.label}</button>; })}</div>
            ...
            <div className="bg-gray-800/50 backdrop-blur rounded-xl border border-gray-700 p-4 mb-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mb-3">
                <div>
                  ...
                  <div className="relative flex items-center gap-1">
                    <input 
                      ...
                      className="flex-1 bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-orange-500 text-white" 
                    />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 pr-16 text-sm focus:outline-none focus:border-orange-500 text-white" 
                    />
                    <select 
                      ...
                      className="absolute right-1 top-1/2 -translate-y-1/2 bg-gray-800 border border-gray-600 rounded px-2 py-1 text-xs focus:outline-none focus:border-orange-500 text-white"
                    >
                ...
                <div>
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => setNewTrade({...newTrade, position: 'long'})} className={`flex-1 py-1.5 rounded-lg text-sm font-medium ${newTrade.position === 'long' ? 'bg-green-900 text-white' : 'bg-gray-700 text-gray-300'}`}>Long</button>
                    <button onClick={() => setNewTrade({...newTrade, position: 'short'})} className={`flex-1 py-1.5 rounded-lg text-sm font-medium ${newTrade.position === 'short' ? 'bg-red-700 text-white' : 'bg-gray-700 text-gray-300'}`}>Short</button>
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-orange-500 text-white" 
                  />
                </div>
                <div>
              ...
              <div className="mb-3">
                ...
                <div className="relative tag-input-container">
                  <div className="flex flex-wrap gap-1.5 w-full bg-gray-900 border border-gray-700 rounded-lg px-2 py-1.5 text-sm focus-within:border-orange-500 min-h-[38px] items-center">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {savedTags.map((tag, idx) => {
                      ...
                      return (
                        ...
                        >
                          <span className={`text-sm ${isPinned ? 'text-green-400 font-semibold' : 'text-green-400'}`}>{tag}</span>
                          {isPinned && (
              ...
              `}</style>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
                  ...
                  <div className="bg-gray-800/50 backdrop-blur rounded-lg p-4 border border-gray-700">
                    ...
                    <p className="text-xl font-bold text-green-400">{stats.profitFactor}R</p>
                      ...
                      {dailyTradeCountAnalysis.map((item) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-3">
                                <div className="w-12 h-12 bg-orange-600/15 rounded-lg flex items-center justify-center">
                                  <span className="text-2xl font-bold text-green-400">{item.limit}</span>
                                </div>
                                <div>
                                  <p className="text-xs text-gray-400">trades/day</p>
                                  {isSelected && selectedDailyCounts.length > 0 && (
                                    <div className="w-5 h-5 bg-orange-600/80 rounded-full flex items-center justify-center mt-1">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="text-xs px-3 py-1 bg-orange-600/80 hover:bg-orange-600 rounded transition"
                      >
                                ...
                                >
                                  ...
                                  {dayData.count > 0 && (
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 whitespace-nowrap">
                                      ...
                                      <div className="text-xs text-green-400 font-bold mt-1">Win Rate: {dayWinRate}%</div>
                                    </div>
                                  )}
                                </div>
                              );
                            ...
                            <div className="flex items-center gap-1 ml-2">
                              <div className={`w-24 text-sm font-bold text-center py-1 rounded ${
                                hourData.totalCount > 0
                                  ? parseFloat(hourWinRate) >= 50 ? 'bg-green-500/10 text-green-400' : 'bg-red-500/20 text-red-400'
                                  : 'text-gray-600'
                              }`}>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="glass-card rounded-xl p-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="w-12 h-12 bg-orange-600/15 rounded-full flex items-center justify-center">
            ...
            <div className="glass-card rounded-xl p-6">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-3xl font-bold text-green-400">{tradingDays}</p>
          ...
          <div className="glass-card rounded-xl p-6 mb-6">
            <p className="text-sm text-gray-300">
              <span className="font-semibold text-green-400">💡 Key Insight:</span> Small stop range ({minStop.toFixed(2)}%-{(minStop + rangeSize).toFixed(2)}%) shows the best balance between stability and profit.
            </p>
          </div>
          ...
          <div className="glass-card rounded-xl p-6">
                    ...
                    stat.trades > 0 && (
                      <tr 
                        ...
                        className={`border-b border-gray-700/50 hover:bg-gray-700/30 transition ${
                          stat.label === bestRange.label ? 'bg-orange-500/10' : ''
                        }`}
                      >
                        ...
                        <td className="text-center py-3 px-4 text-green-400 font-semibold">{stat.avgStop}%</td>
                        <td className="text-center py-3 px-4">
              ...
              <div className="glass-card rounded-xl p-4">
                <div className="flex flex-col gap-3 mb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <input type="text" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-orange-500 w-48 text-white" />
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <select
                      ...
                      className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-orange-500 text-white"
                    >
                    ...
                    {filteredTrades.map((trade) => (
                      <div key={trade.id} className="p-3 bg-gray-900/50 rounded-lg hover:bg-gray-900 transition flex flex-col sm:flex-row items-start sm:items-center justify-between group gap-3">
                        <div className="flex items-center gap-3 flex-1 w-full sm:w-auto">
                          <div className={`w-6 h-6 flex-shrink-0 rounded-full flex items-center justify-center ${trade.result === 'profit' ? 'bg-orange-600/15' : trade.result === 'riskfree' ? 'bg-blue-500/20' : 'bg-red-500/20'}`}>
                            {trade.result === 'profit' ? (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-xs text-gray-400 mt-1">
                              ...
                              {trade.tag && <span className="px-2 py-0.5 rounded bg-green-500/10 text-green-400">#{trade.tag}</span>}
                              {trade.screenshotUrl && (
                                <button 
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
                        <div className="glass-card rounded-xl p-6">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="w-10 h-10 bg-orange-600/15 rounded-full flex items-center justify-center">
                              <Target className="w-5 h-5 text-green-400" />
                            </div>
                          </div>
                          <p className="text-4xl font-bold text-green-400 mb-1">{overallWinRate}%</p>
                        ...
                        <div className="glass-card rounded-xl p-6">
                          ...
                          <p className="text-4xl font-bold text-green-400 mb-1">{avgMonthlyTarget}%</p>
                                ...
                                <div key={monthName} className="bg-gray-900/50 rounded-lg border border-gray-700 overflow-hidden">
                                  <div className="p-5">
                                    <div className="flex items-center justify-between mb-4">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <div className={`px-4 py-1.5 rounded-full text-sm font-bold ${data.totalPnl >= 0 ? 'bg-orange-600/80 text-white' : 'bg-red-500 text-white'}`}>
                                      ...
                                      <div className="bg-gray-800/50 rounded-lg p-3 text-center">
                                        ...
                                        <p className="font-bold text-red-400 text-sm">
                                          -{(() => {
                                            ...
                                            // محاسبه Max DD از بالانس اول ماه
                                            let currentEquity = monthStartBalance;
                                      ...
                                      <div className="bg-gray-800/50 rounded-lg p-3 text-center">
                                        ...
                                        <p className="font-bold text-red-400 text-sm">
                                          -{(() => {
                                            ...
                                            // محاسبه Max DD برای هر روز
                                            Object.values(dailyTrades).forEach(dayTrades => {
                                              // Sort trades by time
                                              dayTrades.sort((a, b) => a.time.localeCompare(b.time));

                                              // فرض: بالانس اول روز معلوم نیست، پس فقط ضررهای پشت سر هم رو حساب می‌کنیم
                                              let currentDayDD = 0;
                                              let runningPnl = 0;
                                              ...
                                              dayTrades.forEach(t => {
                                                runningPnl += t.pnl;
                                                if (runningPnl < 0) {
                                                  const ddPercent = Math.abs((runningPnl / initialBalance) * 100);
                                                  if (ddPercent > currentDayDD) {
                                                    currentDayDD = ddPercent;
                                                  }
                                                }
                                              });
              ...
              <div className="space-y-4">
                <div className="glass-card rounded-xl p-6 border border-gray-700">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h3 className="text-3xl font-bold bg-gradient-to-r from-green-400 to-emerald-600 bg-clip-text text-transparent">
                        Tracking Time
                      </h3>
                  ...
                  <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-700">
                    ...
                    <select
                      ...
                      className="bg-gray-800 border border-gray-600 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-orange-500 text-white"
                    >
                      {(() => {
                        const months = [];
                        const now = new Date();
                        ...
                        days.push(
                          <div
                            ...
                            className={`aspect-square p-3 rounded-xl border-2 transition-all hover:scale-105 ${
                              isToday
                                ? 'bg-gradient-to-br from-green-500/30 to-emerald-500/30 border-orange-500 shadow-lg shadow-green-500/20'
                                : duration > 0
                                ? 'bg-gradient-to-br from-green-500/20 to-emerald-500/20 border-orange-500/50'
                                : 'bg-gray-800/30 border-gray-700 hover:border-gray-600'
                            }`}
                          >
                            <div className="flex flex-col h-full">
                              <div className="text-center mb-auto">
                                <p className={`text-2xl font-bold ${isToday ? 'text-green-400' : 'text-white'}`}>
                                  {day}
                                </p>
                              </div>
                              {duration > 0 && (
                        ...
                        return (
                          <>
                            ...
                            <div className="text-center p-4 bg-orange-500/10 rounded-lg border border-orange-500/30">
                              <p className="text-xs text-green-400 mb-2 font-medium">Active Days</p>
                              <p className="text-3xl font-bold text-green-400">{activeDays}</p>
                              <p className="text-sm text-green-400 mt-1">days tracked</p>
                            </div>
                            <div className="text-center p-4 bg-orange-500/10 rounded-lg border border-orange-500/30">
                              <p className="text-xs text-green-400 mb-2 font-medium">Avg per Day</p>
                              <p className="text-3xl font-bold text-green-400">
                                {Math.floor(avgPerDay / 60000)}m
                              </p>
                              <p className="text-sm text-green-400 mt-1">average</p>
                            </div>
                          </>
                        );
                      })()}
            ...
            {/* MT5 Live Tab - Real-time Trading Dashboard */}
            {activeTab === 'mt5Live' && mt5Mode && (
              <div className="space-y-4">
                <MT5DashboardContent />
              </div>
            )}

            {/* Crypto Live Trades Tab - Real-time Crypto Exchange Dashboard */}
            {activeTab === 'liveTrades' && liveMode && (
              <div className="space-y-4">
                <CryptoLiveTrades />
              </div>
            )}
    ...
    <div className="bg-gray-800 rounded-2xl border border-gray-700 p-6 max-w-md w-full">
        ...
        <div>
          ...
          <div className="flex gap-3">
            <button 
              ...
              className={`flex-1 py-2 rounded-lg font-medium ${newBacktest.balanceType === 'fixed' ? 'bg-orange-600/80 text-white' : 'bg-gray-700 text-gray-300'}`}
            >
              Fixed
            </button>
            <button 
              onClick={() => setNewBacktest({...newBacktest, balanceType: 'dynamic'})} 
              className={`flex-1 py-2 rounded-lg font-medium ${newBacktest.balanceType === 'dynamic' ? 'bg-orange-600/80 text-white' : 'bg-gray-700 text-gray-300'}`}
            >
              Dynamic
            </button>
          </div>
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button 
          ...
          className="flex-1 bg-orange-600/80 hover:bg-orange-600 text-white py-2 rounded-lg font-medium"
        >
          Create
        </button>
      </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={`flex-1 py-2 rounded-lg font-medium ${editingTrade.position === 'long' ? 'bg-orange-600/80 text-white' : 'bg-gray-700 text-gray-300'}`}
            >
              Long
            </button>
            <button 
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={`flex-1 py-2 rounded-lg font-medium ${editingTrade.result === 'profit' ? 'bg-orange-600/80 text-white' : 'bg-gray-700 text-gray-300'}`}
            >
              Profit
            </button>
            <button 
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 w-full bg-gray-900 border border-gray-700 rounded-lg px-2 py-1.5 text-sm focus-within:border-orange-500 min-h-[38px] items-center">
            {(editingTrade.selectedTags || []).map((tag, idx) => (
              <div
                key={idx}
                className="group relative flex items-center gap-1.5 px-2.5 py-1 bg-orange-600/15 border border-orange-500/40 rounded text-xs text-green-400"
              >
                <span>{tag}</span>
                <button
                  onClick={(e) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1 px-3 py-1.5 rounded-lg transition ${
                        isPinned
                          ? 'bg-orange-600/15 border-2 border-orange-500 hover:bg-orange-500/30'
                          : 'bg-orange-600/15 border border-orange-500/40 hover:bg-orange-500/30'
        ...
        {notification.show && (
          <div className="fixed top-4 right-4 z-[60] animate-slide-in">
            <div className={`px-6 py-3 rounded-lg shadow-2xl border-2 flex items-center gap-3 ${
              notification.type === 'profit' ? 'bg-orange-600/15 border-orange-500 backdrop-blur-xl' :
